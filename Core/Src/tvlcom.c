/**
  ******************************************************************************
  * @file    tvlcom.c
  * @brief   UF4COM transport adapter and full data-ID binding.
  ******************************************************************************
  */
#include "tvlcom.h"

#include <string.h>

#include "function.h"
#include "hrtim.h"
#include "tim.h"
#include "uf4com.h"
#include "uf4com_id_table.h"
#include "usart.h"
#include "usbd_cdc_if.h"

#define TVLCOM_CHANNEL_COUNT 3U
#define TVLCOM_STREAM_PERIOD_MS 20U

typedef struct
{
    TVLCOM_Port port;
    uint8_t tx_buffer[TVLCOM_MAX_FRAME_SIZE];
    uint8_t pending_buffer[TVLCOM_MAX_FRAME_SIZE];
    uint16_t pending_len;
    uint8_t tx_busy;
} TVLCOM_ChannelContext;

typedef struct
{
    TVLCOM_Port selected_port;
    uint8_t uart1_rx_buffer[TVLCOM_UART_RX_DMA_SIZE];
    uint8_t uart2_rx_buffer[TVLCOM_UART_RX_DMA_SIZE];
    uint16_t uart1_rx_last_pos;
    uint16_t uart2_rx_last_pos;
    uint8_t fan_manual_enable;
    uint16_t fan_set_permille;
    uint32_t last_fan_apply_tick;
    uint32_t last_stream_tick;
    TVLCOM_ChannelContext channels[TVLCOM_CHANNEL_COUNT];
} TVLCOM_Context;

static TVLCOM_Context s_tvlcom;

static uint16_t s_id_input_voltage;
static uint16_t s_id_input_current;
static uint16_t s_id_output_voltage;
static uint16_t s_id_output_current;
static uint16_t s_id_core_temperature;
static uint16_t s_id_temp1_temperature;
static uint16_t s_id_temp2_temperature;
static uint16_t s_id_set_voltage_limit;
static uint16_t s_id_set_current_limit;
static uint16_t s_id_cc_cv_mode;
static uint16_t s_id_power_state;
static uint16_t s_id_fault_state;
static uint16_t s_id_state_machine_flag_bits;
static uint16_t s_id_state_machine_state;
static uint16_t s_id_input_voltage_raw;
static uint16_t s_id_input_current_raw;
static uint16_t s_id_output_voltage_raw;
static uint16_t s_id_output_current_raw;
static uint16_t s_id_otp_value;
static uint16_t s_id_otp_set_value;
static uint16_t s_id_ovp_value;
static uint16_t s_id_ovp_set_value;
static uint16_t s_id_ocp_value;
static uint16_t s_id_ocp_set_value;
static uint16_t s_id_duty_cmd;
static uint16_t s_id_pwm_a_compare;
static uint16_t s_id_pwm_d_compare;
static uint16_t s_id_fan_speed;
static uint16_t s_id_fan_set_value;
static uint16_t s_id_loop_current_feedback;
static uint16_t s_id_loop_current_reference;
static uint16_t s_id_voltage_loop_current_reference;

static uf4_id_t s_uf4_id_table[] =
{
    {UF4_ID_INPUT_VOLTAGE, &s_id_input_voltage, 0U},
    {UF4_ID_INPUT_CURRENT, &s_id_input_current, 0U},
    {UF4_ID_OUTPUT_VOLTAGE, &s_id_output_voltage, 0U},
    {UF4_ID_OUTPUT_CURRENT, &s_id_output_current, 0U},
    {UF4_ID_CORE_TEMPERATURE, &s_id_core_temperature, 0U},
    {UF4_ID_TEMP1_TEMPERATURE, &s_id_temp1_temperature, 0U},
    {UF4_ID_TEMP2_TEMPERATURE, &s_id_temp2_temperature, 0U},
    {UF4_ID_SET_VOLTAGE_LIMIT, &s_id_set_voltage_limit, 1U},
    {UF4_ID_SET_CURRENT_LIMIT, &s_id_set_current_limit, 1U},
    {UF4_ID_CC_CV_MODE, &s_id_cc_cv_mode, 0U},
    {UF4_ID_POWER_STATE, &s_id_power_state, 1U},
    {UF4_ID_FAULT_STATE, &s_id_fault_state, 0U},
    {UF4_ID_STATE_MACHINE_FLAG_BITS, &s_id_state_machine_flag_bits, 0U},
    {UF4_ID_STATE_MACHINE_STATE, &s_id_state_machine_state, 0U},
    {UF4_ID_INPUT_VOLTAGE_RAW, &s_id_input_voltage_raw, 0U},
    {UF4_ID_INPUT_CURRENT_RAW, &s_id_input_current_raw, 0U},
    {UF4_ID_OUTPUT_VOLTAGE_RAW, &s_id_output_voltage_raw, 0U},
    {UF4_ID_OUTPUT_CURRENT_RAW, &s_id_output_current_raw, 0U},
    {UF4_ID_OTP_VALUE, &s_id_otp_value, 0U},
    {UF4_ID_OTP_SET_VALUE, &s_id_otp_set_value, 1U},
    {UF4_ID_OVP_VALUE, &s_id_ovp_value, 0U},
    {UF4_ID_OVP_SET_VALUE, &s_id_ovp_set_value, 1U},
    {UF4_ID_OCP_VALUE, &s_id_ocp_value, 0U},
    {UF4_ID_OCP_SET_VALUE, &s_id_ocp_set_value, 1U},
    {UF4_ID_DUTY_CMD, &s_id_duty_cmd, 1U},
    {UF4_ID_PWM_A_COMPARE, &s_id_pwm_a_compare, 0U},
    {UF4_ID_PWM_D_COMPARE, &s_id_pwm_d_compare, 0U},
    {UF4_ID_FAN_SPEED, &s_id_fan_speed, 0U},
    {UF4_ID_FAN_SET_VALUE, &s_id_fan_set_value, 1U},
    {UF4_ID_LOOP_CURRENT_FEEDBACK, &s_id_loop_current_feedback, 0U},
    {UF4_ID_LOOP_CURRENT_REFERENCE, &s_id_loop_current_reference, 0U},
    {UF4_ID_VOLTAGE_LOOP_CURRENT_REFERENCE, &s_id_voltage_loop_current_reference, 0U},
};

static TVLCOM_Port s_TVLCOM_GetFixedPort(void)
{
    return (TVLCOM_Port)TVLCOM_FIXED_PORT;
}

static uint8_t s_TVLCOM_IsPortEnabled(TVLCOM_Port port)
{
    return (uint8_t)(port == s_TVLCOM_GetFixedPort());
}

static TVLCOM_ChannelContext *s_TVLCOM_GetChannel(TVLCOM_Port port)
{
    if ((s_TVLCOM_IsPortEnabled(port) == 0U) || ((uint32_t)port >= TVLCOM_CHANNEL_COUNT))
    {
        return NULL;
    }

    return &s_tvlcom.channels[(uint32_t)port];
}

static uint16_t s_TVLCOM_ClampU16(uint32_t value)
{
    return (value > 0xFFFFU) ? 0xFFFFU : (uint16_t)value;
}

static uint16_t s_TVLCOM_FloatToMilliU16(float value)
{
    if(value <= 0.0F)
    {
        return 0U;
    }

    return s_TVLCOM_ClampU16((uint32_t)(value * 1000.0F + 0.5F));
}

static uint16_t s_TVLCOM_FloatToMilliS16(float value)
{
    int32_t milli;

    if(value >= 0.0F)
    {
        milli = (int32_t)(value * 1000.0F + 0.5F);
    }
    else
    {
        milli = (int32_t)(value * 1000.0F - 0.5F);
    }

    if(milli < 0)
    {
        return 0U;
    }
    return s_TVLCOM_ClampU16((uint32_t)milli);
}

static uint16_t s_TVLCOM_GetStateFlagBits(void)
{
    switch(DF.SMFlag)
    {
        case Init:
            return 0x0001U;
        case Wait:
            return 0x0002U;
        case Rise:
            return 0x0004U;
        case Run:
            return 0x0008U;
        case Err:
        default:
            return 0x000FU;
    }
}

static uint16_t s_TVLCOM_GetStateMachineState(void)
{
    switch(DF.BBFlag)
    {
        case Buck:
            return 1U;
        case Boost:
            return 2U;
        case Mix:
            return 3U;
        case NA:
        default:
            return 0U;
    }
}

static uint16_t s_TVLCOM_GetCvccMode(void)
{
    return (CVCC_Mode == CC) ? 0U : 1U;
}

static uint16_t s_TVLCOM_GetDutyCmd(void)
{
    if(DF.BBFlag == Buck)
    {
        return (uint16_t)CtrValue.BuckDuty;
    }
    return (uint16_t)CtrValue.BoostDuty;
}

static uint16_t s_TVLCOM_GetVoltageLoopReferenceMv(void)
{
    if(CtrValue.Vout_ref <= 0)
    {
        return 0U;
    }
    return s_TVLCOM_FloatToMilliU16((float)CtrValue.Vout_ref * REF_3V3 * BSP_POWER_VOUT_SENSE_SCALE / ADC_MAX_VALUE);
}

static uint16_t s_TVLCOM_GetCurrentLoopReferenceMa(void)
{
    return s_TVLCOM_FloatToMilliU16(SET_Value.Iout);
}

static uint16_t s_TVLCOM_GetLoopFeedbackMa(void)
{
    if(IOUT > 0.0F)
    {
        return s_TVLCOM_FloatToMilliU16(IOUT);
    }
    if(IIN > 0.0F)
    {
        return s_TVLCOM_FloatToMilliU16(IIN);
    }
    return 0U;
}

static uint16_t s_TVLCOM_GetFanSpeedPermille(void)
{
    uint32_t compare_value = __HAL_TIM_GET_COMPARE(&htim8, TIM_CHANNEL_3);

    if(compare_value > 1000U)
    {
        compare_value = 1000U;
    }
    return (uint16_t)compare_value;
}

static uint16_t s_TVLCOM_GetFanSetPermille(void)
{
    if(s_tvlcom.fan_manual_enable != 0U)
    {
        return s_tvlcom.fan_set_permille;
    }
    return s_TVLCOM_GetFanSpeedPermille();
}

static void s_TVLCOM_UpdateControlReferenceFromSetting(void)
{
    CtrValue.Vout_SETref = (int32_t)((SET_Value.Vout / BSP_POWER_VOUT_SENSE_SCALE) / REF_3V3 * ADC_MAX_VALUE);
    CtrValue.Iout_ref = (int32_t)(((BSP_POWER_CURRENT_BIAS_V + SET_Value.Iout * BSP_POWER_CURRENT_SENSE_V_PER_A) / REF_3V3) * ADC_MAX_VALUE);
}

static void s_TVLCOM_UpdateReadRegisters(void)
{
    float hottest_temp = Board1_TEMP;

    if(Board2_TEMP > hottest_temp)
    {
        hottest_temp = Board2_TEMP;
    }

    s_id_input_voltage = s_TVLCOM_FloatToMilliU16(VIN);
    s_id_input_current = s_TVLCOM_FloatToMilliU16(IIN);
    s_id_output_voltage = s_TVLCOM_FloatToMilliU16(VOUT);
    s_id_output_current = s_TVLCOM_FloatToMilliU16(IOUT);
    s_id_core_temperature = s_TVLCOM_FloatToMilliS16(CPU_TEMP);
    s_id_temp1_temperature = s_TVLCOM_FloatToMilliS16(Board1_TEMP);
    s_id_temp2_temperature = s_TVLCOM_FloatToMilliS16(Board2_TEMP);
    s_id_set_voltage_limit = s_TVLCOM_FloatToMilliU16(SET_Value.Vout);
    s_id_set_current_limit = s_TVLCOM_FloatToMilliU16(SET_Value.Iout);
    s_id_cc_cv_mode = s_TVLCOM_GetCvccMode();
    s_id_power_state = (uint16_t)(DF.OUTPUT_Flag != 0U ? 1U : 0U);
    s_id_fault_state = DF.ErrFlag;
    s_id_state_machine_flag_bits = s_TVLCOM_GetStateFlagBits();
    s_id_state_machine_state = s_TVLCOM_GetStateMachineState();
    s_id_input_voltage_raw = s_TVLCOM_ClampU16(SADC.Vin);
    s_id_input_current_raw = s_TVLCOM_ClampU16(SADC.Iin);
    s_id_output_voltage_raw = s_TVLCOM_ClampU16(SADC.Vout);
    s_id_output_current_raw = s_TVLCOM_ClampU16(SADC.Iout);
    s_id_otp_value = s_TVLCOM_FloatToMilliS16(hottest_temp);
    s_id_otp_set_value = s_TVLCOM_FloatToMilliS16(MAX_OTP_VAL);
    s_id_ovp_value = s_TVLCOM_FloatToMilliU16(VOUT);
    s_id_ovp_set_value = s_TVLCOM_FloatToMilliU16(MAX_VOUT_OVP_VAL);
    s_id_ocp_value = s_TVLCOM_FloatToMilliU16(IOUT);
    s_id_ocp_set_value = s_TVLCOM_FloatToMilliU16(MAX_VOUT_OCP_VAL);
    s_id_duty_cmd = s_TVLCOM_GetDutyCmd();
    s_id_pwm_a_compare = s_TVLCOM_ClampU16(__HAL_HRTIM_GETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_A, HRTIM_COMPAREUNIT_1));
    s_id_pwm_d_compare = s_TVLCOM_ClampU16(__HAL_HRTIM_GETCOMPARE(&hhrtim1, HRTIM_TIMERINDEX_TIMER_D, HRTIM_COMPAREUNIT_1));
    s_id_fan_speed = s_TVLCOM_GetFanSpeedPermille();
    s_id_fan_set_value = s_TVLCOM_GetFanSetPermille();
    s_id_loop_current_feedback = s_TVLCOM_GetLoopFeedbackMa();
    s_id_loop_current_reference = s_TVLCOM_GetCurrentLoopReferenceMa();
    s_id_voltage_loop_current_reference = s_TVLCOM_GetVoltageLoopReferenceMv();
}

static void s_TVLCOM_ApplyWriteRegisters(void)
{
    float set_voltage = (float)s_id_set_voltage_limit / 1000.0F;
    float set_current = (float)s_id_set_current_limit / 1000.0F;
    float otp = (float)s_id_otp_set_value / 1000.0F;
    float ovp = (float)s_id_ovp_set_value / 1000.0F;
    float ocp = (float)s_id_ocp_set_value / 1000.0F;
    uint16_t fan_set = s_id_fan_set_value;

    if(fan_set > 1000U)
    {
        fan_set = 1000U;
    }

    SET_Value.Vout = set_voltage;
    SET_Value.Iout = set_current;
    MAX_OTP_VAL = otp;
    MAX_VOUT_OVP_VAL = ovp;
    MAX_VOUT_OCP_VAL = ocp;
    SET_Value.SET_modified_flag = 1.0F;
    s_TVLCOM_UpdateControlReferenceFromSetting();

    DF.OUTPUT_Flag = (uint8_t)(s_id_power_state != 0U ? 1U : 0U);
    if(DF.OUTPUT_Flag == 0U)
    {
        DF.PWMENFlag = 0U;
        if(DF.SMFlag != Err)
        {
            DF.SMFlag = Wait;
        }
    }

    s_tvlcom.fan_manual_enable = 1U;
    s_tvlcom.fan_set_permille = fan_set;
    FAN_PWM_set((uint16_t)(fan_set / 10U));
    s_tvlcom.last_fan_apply_tick = HAL_GetTick();
}

static HAL_StatusTypeDef s_TVLCOM_SendOnChannel(TVLCOM_ChannelContext *channel, const uint8_t *data, uint16_t len)
{
    if((channel == NULL) || (data == NULL) || (len == 0U) || (len > TVLCOM_MAX_FRAME_SIZE))
    {
        return HAL_ERROR;
    }

    if(channel->tx_busy != 0U)
    {
        if((channel->port == TVLCOM_PORT_USART1) && (huart1.gState == HAL_UART_STATE_READY))
        {
            channel->tx_busy = 0U;
        }
        else if((channel->port == TVLCOM_PORT_USART2) && (huart2.gState == HAL_UART_STATE_READY))
        {
            channel->tx_busy = 0U;
        }
    }

    if(channel->tx_busy != 0U)
    {
        return HAL_BUSY;
    }

    memcpy(channel->tx_buffer, data, len);

    switch(channel->port)
    {
        case TVLCOM_PORT_USART1:
            if(HAL_UART_Transmit_DMA(&huart1, channel->tx_buffer, len) != HAL_OK)
            {
                return HAL_BUSY;
            }
            channel->tx_busy = 1U;
            return HAL_OK;

        case TVLCOM_PORT_USART2:
            if(HAL_UART_Transmit_DMA(&huart2, channel->tx_buffer, len) != HAL_OK)
            {
                return HAL_BUSY;
            }
            channel->tx_busy = 1U;
            return HAL_OK;

        case TVLCOM_PORT_CDC:
        default:
            if(CDC_Transmit_FS(channel->tx_buffer, len) != USBD_OK)
            {
                return HAL_BUSY;
            }
            channel->tx_busy = 1U;
            return HAL_OK;
    }
}

static HAL_StatusTypeDef s_TVLCOM_QueueBytes(TVLCOM_ChannelContext *channel, const uint8_t *data, uint16_t len)
{
    if((channel == NULL) || (data == NULL) || (len == 0U) || (len > TVLCOM_MAX_FRAME_SIZE))
    {
        return HAL_ERROR;
    }

    if(s_TVLCOM_SendOnChannel(channel, data, len) == HAL_OK)
    {
        return HAL_OK;
    }

    memcpy(channel->pending_buffer, data, len);
    channel->pending_len = len;
    return HAL_OK;
}

static void s_TVLCOM_FlushPending(TVLCOM_ChannelContext *channel)
{
    if((channel == NULL) || (channel->pending_len == 0U))
    {
        return;
    }

    if(s_TVLCOM_SendOnChannel(channel, channel->pending_buffer, channel->pending_len) == HAL_OK)
    {
        channel->pending_len = 0U;
    }
}

static void s_TVLCOM_PollTxCompletion(TVLCOM_ChannelContext *channel)
{
    if((channel == NULL) || (channel->tx_busy == 0U))
    {
        return;
    }

    if((channel->port == TVLCOM_PORT_USART1) && (huart1.gState == HAL_UART_STATE_READY))
    {
        channel->tx_busy = 0U;
    }
    else if((channel->port == TVLCOM_PORT_USART2) && (huart2.gState == HAL_UART_STATE_READY))
    {
        channel->tx_busy = 0U;
    }
}

static void s_TVLCOM_Uf4Tx(const uint8_t *data, uint16_t len, void *user)
{
    (void)user;
    (void)TVLCOM_SendBytes(data, len);
}

static void s_TVLCOM_Uf4WriteApply(void *user)
{
    (void)user;
    s_TVLCOM_ApplyWriteRegisters();
    s_TVLCOM_UpdateReadRegisters();
}

static void s_TVLCOM_ProcessUartRxWindow(TVLCOM_Port port, uint8_t *buffer, uint16_t *last_pos, uint16_t current_pos)
{
    if(current_pos > TVLCOM_UART_RX_DMA_SIZE)
    {
        current_pos = TVLCOM_UART_RX_DMA_SIZE;
    }

    if(current_pos > *last_pos)
    {
        (void)UF4_InputBuffer(&buffer[*last_pos], (uint16_t)(current_pos - *last_pos));
    }
    else if(current_pos < *last_pos)
    {
        (void)UF4_InputBuffer(&buffer[*last_pos], (uint16_t)(TVLCOM_UART_RX_DMA_SIZE - *last_pos));
        if(current_pos > 0U)
        {
            (void)UF4_InputBuffer(buffer, current_pos);
        }
    }
    else if(current_pos == TVLCOM_UART_RX_DMA_SIZE)
    {
        (void)UF4_InputBuffer(&buffer[*last_pos], (uint16_t)(TVLCOM_UART_RX_DMA_SIZE - *last_pos));
    }

    (void)port;
    *last_pos = (current_pos >= TVLCOM_UART_RX_DMA_SIZE) ? 0U : current_pos;
}

static void s_TVLCOM_PollUartRx(void)
{
    uint16_t current_pos;

    if((s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART1) != 0U) && (huart1.hdmarx != NULL))
    {
        current_pos = (uint16_t)(TVLCOM_UART_RX_DMA_SIZE - __HAL_DMA_GET_COUNTER(huart1.hdmarx));
        s_TVLCOM_ProcessUartRxWindow(TVLCOM_PORT_USART1, s_tvlcom.uart1_rx_buffer, &s_tvlcom.uart1_rx_last_pos, current_pos);
    }

    if((s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART2) != 0U) && (huart2.hdmarx != NULL))
    {
        current_pos = (uint16_t)(TVLCOM_UART_RX_DMA_SIZE - __HAL_DMA_GET_COUNTER(huart2.hdmarx));
        s_TVLCOM_ProcessUartRxWindow(TVLCOM_PORT_USART2, s_tvlcom.uart2_rx_buffer, &s_tvlcom.uart2_rx_last_pos, current_pos);
    }
}

static void s_TVLCOM_StartUartRxDma(UART_HandleTypeDef *huart, uint8_t *buffer)
{
    if(HAL_UART_Receive_DMA(huart, buffer, TVLCOM_UART_RX_DMA_SIZE) == HAL_OK)
    {
        if(huart->hdmarx != NULL)
        {
            __HAL_DMA_DISABLE_IT(huart->hdmarx, DMA_IT_HT);
        }
    }
}

void TVLCOM_Init(void)
{
    uint32_t i;

    memset(&s_tvlcom, 0, sizeof(s_tvlcom));
    s_tvlcom.selected_port = s_TVLCOM_GetFixedPort();

    for(i = 0U; i < TVLCOM_CHANNEL_COUNT; ++i)
    {
        s_tvlcom.channels[i].port = (TVLCOM_Port)i;
    }

    s_TVLCOM_UpdateReadRegisters();
    UF4_IDTableBind(s_uf4_id_table, sizeof(s_uf4_id_table) / sizeof(s_uf4_id_table[0]));
    UF4_Init(s_TVLCOM_Uf4Tx, NULL);
    UF4_SetWriteApplyCallback(s_TVLCOM_Uf4WriteApply, NULL);

    if(s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART1) != 0U)
    {
        s_TVLCOM_StartUartRxDma(&huart1, s_tvlcom.uart1_rx_buffer);
    }
    if(s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART2) != 0U)
    {
        s_TVLCOM_StartUartRxDma(&huart2, s_tvlcom.uart2_rx_buffer);
    }
}

void TVLCOM_RunTask(void)
{
    uint32_t i;
    const uint32_t tick_now = HAL_GetTick();

    s_TVLCOM_UpdateReadRegisters();
    s_TVLCOM_PollUartRx();

    if((s_tvlcom.fan_manual_enable != 0U) && ((tick_now - s_tvlcom.last_fan_apply_tick) >= 50U))
    {
        FAN_PWM_set((uint16_t)(s_tvlcom.fan_set_permille / 10U));
        s_tvlcom.last_fan_apply_tick = tick_now;
    }

    for(i = 0U; i < TVLCOM_CHANNEL_COUNT; ++i)
    {
        if(s_TVLCOM_IsPortEnabled(s_tvlcom.channels[i].port) != 0U)
        {
            s_TVLCOM_PollTxCompletion(&s_tvlcom.channels[i]);
            s_TVLCOM_FlushPending(&s_tvlcom.channels[i]);
        }
    }

    if((tick_now - s_tvlcom.last_stream_tick) >= TVLCOM_STREAM_PERIOD_MS)
    {
        s_tvlcom.last_stream_tick = tick_now;
        UF4_Process();
    }
}

void TVLCOM_SelectPort(TVLCOM_Port port)
{
    if(s_TVLCOM_IsPortEnabled(port) != 0U)
    {
        s_tvlcom.selected_port = port;
    }
}

TVLCOM_Port TVLCOM_GetPort(void)
{
    return s_TVLCOM_GetFixedPort();
}

HAL_StatusTypeDef TVLCOM_SendBytes(const uint8_t *data, uint16_t len)
{
    return s_TVLCOM_QueueBytes(s_TVLCOM_GetChannel(s_tvlcom.selected_port), data, len);
}

void TVLCOM_OnUsbCdcRx(const uint8_t *data, uint16_t len)
{
    if((data == NULL) || (len == 0U) || (s_TVLCOM_IsPortEnabled(TVLCOM_PORT_CDC) == 0U))
    {
        return;
    }

    s_tvlcom.selected_port = TVLCOM_PORT_CDC;
    s_TVLCOM_UpdateReadRegisters();
    (void)UF4_InputBuffer(data, len);
    s_TVLCOM_UpdateReadRegisters();
}

void USER_tvlcomTransportOnUsbCdcRx(uint8_t *data, uint16_t len)
{
    TVLCOM_OnUsbCdcRx(data, len);
}

void TVLCOM_OnTxComplete(TVLCOM_Port port)
{
    TVLCOM_ChannelContext *channel = s_TVLCOM_GetChannel(port);

    if(channel != NULL)
    {
        channel->tx_busy = 0U;
    }
}

void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    (void)huart;
    (void)Size;
}

void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if(huart == &huart1)
    {
        TVLCOM_OnTxComplete(TVLCOM_PORT_USART1);
        s_tvlcom.uart1_rx_last_pos = 0U;
        if(s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART1) != 0U)
        {
            s_TVLCOM_StartUartRxDma(&huart1, s_tvlcom.uart1_rx_buffer);
        }
    }
    else if(huart == &huart2)
    {
        TVLCOM_OnTxComplete(TVLCOM_PORT_USART2);
        s_tvlcom.uart2_rx_last_pos = 0U;
        if(s_TVLCOM_IsPortEnabled(TVLCOM_PORT_USART2) != 0U)
        {
            s_TVLCOM_StartUartRxDma(&huart2, s_tvlcom.uart2_rx_buffer);
        }
    }
}

void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart == &huart1)
    {
        TVLCOM_OnTxComplete(TVLCOM_PORT_USART1);
    }
    else if(huart == &huart2)
    {
        TVLCOM_OnTxComplete(TVLCOM_PORT_USART2);
    }
}
